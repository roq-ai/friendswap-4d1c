...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<SwapperInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/swappers` : null),
    () => getSwappers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Swapper',
      path: isTenantUser ? `/swappers/view/${data?.[0]?.id}` : '/swappers',
      entity: 'swapper',
