...
import { getSwapperById, updateSwapperById } from 'apiSdk/swappers';
...
function SwapperEditPage() {
  ...
  const handleSubmit = async (values: SwapperInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSwapperById(id, values);
      mutate(updated);
      resetForm();
      router.push('/swappers');
    } catch (error) {
      setFormError(error);
    }
  };

  const formik = useFormik<SwapperInterface>({
    initialValues: data,
